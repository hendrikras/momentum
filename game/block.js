...
function connectSeesawBlocks() {
  ...
  Object.values(seesawGroups).forEach(seesawGroup => {
    ...
    const newSeesawBody = Bodies.fromVertices(pivotX, pivotY, seesawPoints, {
      isStatic: false,
      friction: 0.5,
      frictionAir: pivotBlock.body.frictionAir,
      density: 0.1,
      restitution: 0.2,
    });
    ...
    // Set angular damping
    newSeesawBody.angularDamping = pivotBlock.body.angularDamping;
...
// Add this inside the Block constructor, after the isPivoting assignment
...
// In the run() method, replace the existing seesaw physics code with this:
    if (col && col.supports) {
      ...
      if (this.isPivoting) {
        ...
        // Increase torque factor for more responsive tilting
        let torque = relativeX * playerMass * 0.0015;
